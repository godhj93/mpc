...
# Zero since we have a single pendulum system
m2 = 0.0  # kg, mass of the second rod
L2 = 0.0  # m,  length of the second rod
...
euler_lagrange = vertcat(
        # 1
        h1*ddpos+h2*ddtheta[0]*cos(theta[0]) - (h2*dtheta[0]**2*sin(theta[0]) + u),
        # 2
        h2*cos(theta[0])*ddpos + h4*ddtheta[0] - (h7*sin(theta[0])),
        # 3
...
anim.save('anim_sip.mp4', writer=ffmpeg_writer)  # Saving as an MP4 file
